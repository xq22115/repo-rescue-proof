...
function ConvertTo-UtcTimestamp([object]$Value, [string]$FieldPath) {
    ...
    $text = [string]$Value
    Assert-True (-not [string]::IsNullOrWhiteSpace($text)) "$FieldPath is required."
...
$expectedStartTime = ConvertTo-UtcTimestamp -Value (Get-PropertyValue $observedProcess 'startTimeUtc') -FieldPath 'spawnedProcessIdentity.observedProcess.startTimeUtc'
$observedReceiptImagePath = [System.IO.Path]::GetFullPath([string](Get-PropertyValue $observedProcess 'imagePath'))
$observedReceiptImageSha = Assert-Sha256 -Value ([string](Get-PropertyValue $observedProcess 'imageBackingFileSha256')) -FieldPath 'spawnedProcessIdentity.observedProcess.imageBackingFileSha256'
Assert-True ($observedReceiptImagePath.Equals($expectedPath, [System.StringComparison]::OrdinalIgnoreCase)) 'Spawned-process receipt observed image path no longer agrees with its expected executable path.'
Assert-True ($observedReceiptImageSha -eq $expectedExecutableSha) 'Spawned-process receipt observed image SHA-256 no longer agrees with its expected executable SHA-256.'
