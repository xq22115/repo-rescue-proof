...
$receiptPath=[IO.Path]::GetFullPath([string](Get-PropertyValue $o 'imagePath'))
$receiptSha=Assert-Sha256 ([string](Get-PropertyValue $o 'imageBackingFileSha256')) 'observedProcess.imageBackingFileSha256'
Assert-True ($receiptPath.Equals($expectedPath,[StringComparison]::OrdinalIgnoreCase)) 'Spawned receipt image path conflicts with expected executable.'
Assert-True ($receiptSha -eq $expectedExeSha) 'Spawned receipt image SHA conflicts with expected executable.'
